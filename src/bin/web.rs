...
async fn handle_socket(mut socket: axum::extract::ws::WebSocket, state: AppState) {
    let Ok(streams) = state.store.list_streams().await else { return };
    
    for stream in streams {
        state.store.ensure_tail(&stream).await;
    }
    ...
    state.store.stop_tails_if_idle().await;
}
...
async fn main() -> Result<()> {
    ...
    let state = AppState { store, allowed: filter };
